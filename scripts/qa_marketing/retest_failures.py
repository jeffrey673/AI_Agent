"""Re-test failed queries from Marketing QA 3,900."""
import json
import time
...
RETEST_FILE = BASE_DIR / "retest_results.json"


def load_failures():
    """Load all FAIL/ERROR/EMPTY from results."""
    fails = []
    for f in sorted(RESULTS_DIR.glob("results_*.json")):
        table = f.stem.replace("results_", "")
...
def main():
    fails = load_failures()
    print(f"Re-testing {len(fails)} failed queries...")
    ...
    for i, r in enumerate(fails):
        ...
        print(
            f"  [{i+1}/{len(fails)}] [{icon}] {result['id']:8s} {result['time']:5.1f}s "
    ...
    avg_new = sum(r["time"] for r in results) / len(results)
